<script>
    ...
    const broswer = detect();
    console.log("LEBRONJAMES:" + broswer.name)
</script>
...
<Modal bind:open={signupModal} size="xs" autoclose={false} class="w-full rounded-2xl">
    <form class="flex flex-col space-y-6" action="#">
      ...
      <Label class="space-y-2">
        <span>Email</span>
        <Input type="email" name="email" placeholder="name@youremail.com" required />
      </Label>
      <Label class="space-y-2">
        <span>Your password</span>
        <Input type="password" name="password" placeholder="••••••••" required />
      </Label>
      <Button type="submit" class="w-full1">Create a new account</Button>
    </form>
</Modal>

<!-- INITIAL CTA (W HERO) -->
<div class="flex flex-row items-center justify-evenly mt-20">
    <div class="max-w-xs lg:max-w-sm flex flex-col text-center items-center md:text-left md:items-start">
    ...
    <img src={hero_image} alt="TaskBlitz Hero" class="h-128 hidden md:block"/>
</div>
